...
class ProccessController extends Controller
{
   ...
   //dd($bentrokdatarpl);
    ...
    ->select('hari.nama AS Hari', 'jam.kode AS kode_jam', 'jam.range_jam as jam','jadwalkuliah.kode_hari as kdhari','jadwalkuliah.kode_jam as kdjam',
    ...
    ->leftJoin('ruang', 'jadwalkuliah.kode_ruang', '=', 'ruang.kode')
    ->leftJoin('kelas','pengampu.kelas','=','kelas.nama')
    ->select('hari.nama AS Hari', 'jam.kode AS kode_jam', 'jam.range_jam as jam','jadwalkuliah.kode_hari as kdhari','jadwalkuliah.kode_jam as kdjam','matakuliah.sks as sks', 'matakuliah.nama as nmmatkul','pengampu.kelas as kelas',
    'dosen.nama as nmdos','ruang.nama as nmruang')
    ...
    foreach ($viewrpl as $row) {
        ...
        if ($matkulIndex === false) {
            ...
            return view('schedule.index', [ 'bentrok'=>$bentrok, 'tidakBentrok' => $tidakBentrok, 'bentrokdata'=> $bentrokdata, 'tidakBentrokrpl' => $tidakBentrokrpl, 'bentrokdatarpl'=> $bentrokdatarpl, 'jadwal' => $jadwal, 'kelasti' => $kelasti, 'viewti' =>$viewti, 'kelasrpl' => $kelasrpl, 'viewrpl' =>$viewrpl,'prodi' => $prodi, 'tahun' => $tahun, 'jam' =>$jam, 'uniqueJamCodes' =>$uniqueJamCodes, 'harirpl'=> $harirpl, 
            'tableData' => $tableData]);
        }
...
// dd($existingSchedule);
 ...
 ->leftJoin('ruang', 'jadwalkuliah.kode_ruang', '=', 'ruang.kode')
 ->leftJoin('kelas','pengampu.kelas','=','kelas.nama')
 ->select('hari.nama AS Hari', 'jam.kode AS kode_jam', 'jam.range_jam as jam',
 DB::raw("MAX(CASE WHEN pengampu.kelas = 'D3TI1A' THEN CONCAT(matakuliah.nama, '\n ', dosen.nama, '\n ', ruang.nama, ', ', matakuliah.sks, ' SKS') ELSE NULL END) AS D3TI1A"),
 DB::raw("MAX(CASE WHEN pengampu.kelas = 'D3TI1B' THEN CONCAT(matakuliah.nama, '\n', dosen.nama, '\n ', ruang.nama, ', ', matakuliah.sks, ' SKS') ELSE NULL END) AS D3TI1B"),
 DB::raw("MAX(CASE WHEN pengampu.kelas = 'D3TI1C' THEN CONCAT(matakuliah.nama, '\n', dosen.nama, '\n', ruang.nama, ', ', matakuliah.sks, ' SKS') ELSE NULL END) AS D3TI1C"),
     DB::raw("MAX(CASE WHEN pengampu.kelas = 'D3TI2A' THEN CONCAT(matakuliah.nama, '\n ', dosen.nama, '\n ', ruang.nama, ', ', matakuliah.sks, ' SKS') ELSE NULL END) AS D3TI2A"),
